 ...
 * choose 8u-dev (or the concrete release, like 8u40) rt (land in log)
 * navigate to tags (list of tags) and click what's wanted (land in changeset)
 * navigate to browse (land in file dir)
 * now modules/controls/src/test and down the package path
 *
 * This particular is
 * http://hg.openjdk.java.net/openjfx/8u40/rt/file/298ec4ec6b35/modules/controls/src/test/java/javafx/scene/control/ListViewTest.java
 * was locked in Feb, 11, 2015 
 * 
 * This is the original adjusted to use SimpleSelectionModel, with commented tests for 
 * - accessibility, 
 * - VirtualFlowTest
 * these methods fail instead, with an ignoreRule to turn the failures on/off
 * 
 * - all ListView instantiation is extracted into two factory methods, to
 * hook SimpleListViewSelectionModel. 
 * - Person imported from demobeans.
 * 
 */
@SuppressWarnings({ "rawtypes", "unchecked" })
@RunWith(JUnit4.class)
public class ListViewTestSimple8u40Feb2015 {
    @ClassRule
...
//        assertStyleClassContains(b2, "list-view");
    ...
    @Test public void selectionModelCanBeBound() {
        fail("TBD - ListViewBitSelectionModel");
...
//        assertSame(sm, sm);
    ...
    /**
     * This fails for simpleSelectionModel: ambiguity if list containing 
     * a single item is replaced by a list with a single item - no way
     * to distinguish a items.setAll(item) from items.set(0, item).
     */
    ...
    /**
     * IllegalStateException in indexMappedList: got a change of no type
     * with string: { [] added at 0 }
     * Might be due to binding to listProperty: it fires such an event 
     * on setting an empty list?
     */
...
//        assertEquals(0, rt_35889_cancel_count);
    ...
    /**
     * Fails due to unresolved strategy for removeAtSelected
     */
